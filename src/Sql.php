...
class Sql
{
    ...
    public static function read(SqlDataSource $ds, $mode, $args, $cb=null)
    {
        ...
        switch ($mode) {
            case self::MODE_READ_GETALL: // array(sql,limit,counted),如果sql里面带了limit则不能使用limit
        ...
        return (empty($cb) || !is_callable($cb)) ? $result : call_user_func_array($cb, [$result]);
    }
